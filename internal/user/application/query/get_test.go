...
func TestGetHandler_Success(t *testing.T) {
	now := time.Now()
	u, _ := entity.ReconstructUser(testUUID, "test@example.com", "hashed", "Test User", now, now)
	h := NewGetHandler(&mockUserRepo{user: u})

	result, err := h.Handle(context.Background(), Get{UserID: testUUID})
	if err != nil {
...
func TestGetHandler_NotFound(t *testing.T) {
	...
	_, err := h.Handle(context.Background(), Get{UserID: testUUID})
	if err == nil {
