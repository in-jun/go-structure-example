...
func (r *pgReader) FindByPaymentID(ctx context.Context, paymentID string) ([]domainEvent.StoredEvent, error) {
	db := r.dbGetter(ctx)
	rows, err := db.QueryContext(ctx,
		"SELECT id, aggregate_id, event_type, payload, occurred_at FROM domain_events WHERE aggregate_type = 'payment' AND aggregate_id = $1 ORDER BY id",
		paymentID,
	)
	if err != nil {
